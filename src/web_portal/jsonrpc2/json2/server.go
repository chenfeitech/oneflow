...
// NewRequest returns a CodecRequest.
func (c *Codec) NewRequestWithBytes(r []byte) rpc.CodecRequest {
	return newCodecRequestWithBytes(r, c.encSel.Select(nil))
}
...
// newCodecRequest returns a new CodecRequest.
func newCodecRequestWithBytes(r []byte, encoder rpc.Encoder) rpc.CodecRequest {
	// Decode the request body and check if RPC method is valid.
	req := new(serverRequest)
...
func (c *CodecRequest) WriteResponse(w io.Writer, reply interface{}) {
	res := &serverResponse{
		Version: Version,
		Result:  reply,
		Id:      c.request.Id,
...
func (c *CodecRequest) WriteError(w io.Writer, status int, err error) {
...
func (c *CodecRequest) writeServerResponse(w io.Writer, res *serverResponse) {
